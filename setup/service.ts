...
<dict>
    ...
    <string>${projectRoot}/logs/nanoflash.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/nanoflash.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.nanoflash');
  } catch {
...
/**
 * Kill any orphaned nanoflash node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned nanoflash processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/nanoflash.service';
    ...
    unitPath = path.join(unitDir, 'nanoflash.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=NanoFlash Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/nanoflash.log
StandardError=append:${projectRoot}/logs/nanoflash.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned nanoflash processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable nanoflash`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start nanoflash`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active nanoflash`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-nanoflash.sh');
  const pidFile = path.join(projectRoot, 'nanoflash.pid');

  const lines = [
    '#!/bin/bash',
    '# start-nanoflash.sh — Start NanoFlash without systemd',
    ...
    '    echo "Stopping existing NanoFlash (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting NanoFlash..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/nanoflash.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/nanoflash.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "NanoFlash started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/nanoflash.log"`,
  ];
