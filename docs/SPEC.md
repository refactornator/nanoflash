...
│  │    • mcp__nanoflash__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
│   ├── nanoflash.log               # Host stdout
│   └── nanoflash.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.nanoflash.plist         # macOS service configuration
```
...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'Sergey';
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'nanoflash-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your nanoflash installation
...
NanoFlash uses a hierarchical memory system based on CLAUDE.md files.
...
7. Router invokes Claude Agent SDK:
   ...
   └── mcpServers: nanoflash (scheduler)
   │
   ▼
...
Messages must start with the trigger pattern (default: `@Sergey`):
- `@Sergey what's the weather?` → ✅ Triggers Claude
- `@andy help me` → ✅ Triggers (case insensitive)
- `Hey @Sergey` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @Sergey what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@Sergey what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@Sergey add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@Sergey remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@Sergey list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@Sergey remember I prefer dark mode` | Add to global memory |

---

## Scheduled Tasks

NanoFlash has a built-in scheduler that runs tasks as full agents in their group's context.
...
```
User: @Sergey remind me every Monday at 9am to review the weekly metrics

Claude: [calls mcp__nanoflash__schedule_task]
        {
...
```
User: @Sergey at 5pm today, send me a summary of today's emails
...
From any group:
- `@Sergey list my scheduled tasks` - View tasks for this group
- `@Sergey pause task [id]` - Pause a task
- `@Sergey resume task [id]` - Resume a paused task
- `@Sergey cancel task [id]` - Delete a task

From main channel:
- `@Sergey list all tasks` - View tasks from all groups
- `@Sergey schedule task for "Family Chat": [prompt]` - Schedule for another group

---

## MCP Servers

### NanoFlash MCP (built-in)

The `nanoflash` MCP server is created dynamically per agent call with the current group's context.
...
NanoFlash runs as a single macOS launchd service.

### Startup Sequence

When NanoFlash starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned NanoFlash containers from previous runs
...
### Service: com.nanoflash

**launchd/com.nanoflash.plist:**
```xml
...
<dict>
    ...
    <dict>
        ...
        <string>Sergey</string>
    </dict>
    <key>StandardOutPath</key>
    <string>{{PROJECT_ROOT}}/logs/nanoflash.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/nanoflash.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.nanoflash.plist ~/Library/LaunchAgents/
...
# Check status
launchctl list | grep nanoflash

# View logs
tail -f logs/nanoflash.log
```
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep nanoflash` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; NanoFlash auto-starts container runtime but may fail |
| "Claude Code process exited with code 1" | Session mount path wrong | Ensure mount is to `/home/node/.claude/` not `/root/.claude/` |
| Session not continuing | Session ID not saved | Check SQLite: `sqlite3 store/messages.db "SELECT * FROM sessions"` |
| Session not continuing | Mount path mismatch | Container user is `node` with HOME=/home/node; sessions must be at `/home/node/.claude/` |
| "QR code expired" | WhatsApp session expired | Delete store/auth/ and restart |
| "No groups registered" | Haven't added groups | Use `@Sergey add group "Name"` in main |

### Log Location

- `logs/nanoflash.log` - stdout
- `logs/nanoflash.error.log` - stderr
