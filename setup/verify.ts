...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.nanoflash')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.nanoflash'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active nanoflash`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
  ...
  if (
    fs.existsSync(
      path.join(homeDir, '.config', 'nanoflash', 'mount-allowlist.json'),
    )
  ) {
